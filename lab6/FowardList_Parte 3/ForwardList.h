...
public:
    ...
    {
        Node *aux = new Node(val, position.m_pointer->next);

        position.m_pointer->next = aux;
    ...
    {
        Node *aux = position.m_pointer->next;

        if (aux != nullptr)
        {
            position.m_pointer->next = aux->next;
            delete aux;
        }

        m_size--;

        return Iterator(position.m_pointer->next);
    }
};
