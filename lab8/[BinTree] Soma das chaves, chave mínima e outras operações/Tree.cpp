...
{ // TODO
    ...
    if (node->left == nullptr)
        return node->key < _min_key(node->right) ? node->key : _min_key(node->right);

    if (node->right == nullptr)
        return node->key < _min_key(node->left) ? node->key : _min_key(node->left);

    if (node->key < _min_key(node->left) && node->key < _min_key(node->right))
        return node->key;

    else
    {
        if (_min_key(node->left) < _min_key(node->right))
