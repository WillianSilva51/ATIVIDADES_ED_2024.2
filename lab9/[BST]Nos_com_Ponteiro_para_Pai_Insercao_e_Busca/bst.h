...
bool BST::add(int k)
{
    ...
    if (k < previous->key)
        previous->left = new Node(k, nullptr, nullptr, previous);

    else if (k > previous->key)
        previous->right = new Node(k, nullptr, nullptr, previous);
